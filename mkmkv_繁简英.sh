...
while IFS= read -r file; do
  ...
  if [ -f "$file" ]; then
    ...
    if mkvmerge --quiet \
      ...
      -s 2,3,4 \
      --sub-charset 2:UTF-8 --language 2:zh-Hant --track-name 2:繁體中文 --default-track-flag 2:no \
      --sub-charset 3:UTF-8 --language 3:zh-Hans --track-name 3:简体中文 \
      --sub-charset 4:UTF-8 --language 4:en --track-name 4:English --default-track-flag 4:no \
      "$file" \
      --track-order 0:0,0:1,0:3,0:2,0:4 2>&1; then
      success=$((success + 1))
    else
      failed=$((failed + 1))
      echo "  错误: 处理失败"
