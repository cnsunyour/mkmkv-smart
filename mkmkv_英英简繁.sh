...
while IFS= read -r file; do
  ...
  if [ -f "$file" ]; then
    ...
    if mkvmerge --quiet \
      ...
      --sub-charset 3:UTF-8 --language 3:en --track-name 3:English --default-track-flag 3:no \
      --sub-charset 4:UTF-8 --language 4:zh-Hans --track-name 4:简体中文 \
      --sub-charset 5:UTF-8 --language 5:zh-Hant --track-name 5:繁體中文 \
      "$file" \
      --track-order 0:0,0:1,0:4,0:5,0:2,0:3 2>&1; then
      success=$((success + 1))
    else
      failed=$((failed + 1))
      echo "  错误: 处理失败"
